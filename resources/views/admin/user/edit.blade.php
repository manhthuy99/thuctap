...
@endsection
@section('content')
   <form id="user_edit_form" action="{{ route('user.update',$user->username) }}" method="post" >
      {{ csrf_field() }}
      {{ method_field('put') }}

      <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
         <label class="bolder bigger-110" for="name">Tên</label>
         <input type="text" name="name" maxlength="21" id="name" placeholder="Tên"
      ...
      @if ($user->address)
         <div class="form-group">
            <a href="{{ route('admin.address.edit',$user->username) }}" class="btn btn-lg btn-success">Edit Address</a>
         </div>
      @endif
      --}}
   </form>
      ...
      <script type="text/javascript">
          $(document).ready(function ()
          {
              $("#user_edit_form").submit(function (e) {
                  ...
                  if (upload_ajax("{{ route('user.update',$user->username) }}",data))
                  {
